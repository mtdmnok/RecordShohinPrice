...
public class LoginController {
	...
	public String login(ModelMap modelMap, @RequestParam("ID") String ID, @RequestParam("password") String password) {
		...
		//ここでloadUserByUsernameを呼び出す？
		DemoUserDetailsService userService = new DemoUserDetailsService();
		UserDetails loginInfoList = userService.loadUserByUsername(ID);
		
		//パスワードが一致しているかチェックする?
		if (password == loginInfoList.getPassword()) {
			loginFlg = true;
		}
